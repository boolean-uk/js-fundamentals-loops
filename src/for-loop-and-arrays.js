...
const doubledNums = []
for (let i = 0; i < nums.length; i++) doubledNums.push(nums[i] * 2)

// 3. Use a for loop to set word equal to all the letters in the letters array
for (let i = 0; i < letters.length; i++)word += letters[i]
...
const numsReversed = []
for (let i = nums.length - 1; i >= 0; i--) numsReversed.push(nums[i])
